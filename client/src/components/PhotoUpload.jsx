      ...
      {webcamStream ? (
        <div className="relative rounded-2xl overflow-hidden">
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
            <button onClick={captureWebcam} className="px-6 py-3 bg-primary-500 text-white rounded-full font-semibold hover:bg-primary-600 transition-colors">
              📸 Capture
            </button>
            <button onClick={() => { webcamStream.getTracks().forEach(t => t.stop()); setWebcamStream(null) }} className="px-4 py-3 bg-gray-800/70 text-white rounded-full font-semibold hover:bg-gray-800 transition-colors">
              ✕ Cancel
            </button>
          </div>
        </div>
